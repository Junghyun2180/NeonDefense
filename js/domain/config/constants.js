...
// ===== 캐리오버 설정 =====
// 코어 정체성(스테이지별 메타 변화)에 맞춰 maxTowers 5→4로 1슬롯 축소.
// 새 스테이지의 적 테마에 적응할 슬롯 여유를 확보하면서, 캐리오버 도파민은
// 4슬롯으로도 충분히 보존된다. 서포트는 가격이 비싸 자연 제한이 있어 그대로.
const CARRYOVER = {
  maxTowers: 4,        // 5→4 (스테이지 메타 변화 도입에 따른 적응 슬롯 확보)
  maxSupports: 3,
