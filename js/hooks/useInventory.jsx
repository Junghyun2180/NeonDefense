...
const useInventory = (gameState, settings = {}) => {
    ...
    // 사이드이펙트(setGold) 를 setInventory 업데이터 안에 두지 않기 위해 외부 inventory 로 미리 plan 계산.
    const bulkSellInventoryTowers = useCallback((elementFilter = null) => {
        ...
        if (tiers.length < 2) return; // 한 티어만 있으면 보존
        ...
        setInventory(prev => prev.filter(n => !sellIds.has(n.id)));
        setGold(g => g + refund);
        setSelectedInventory([]);
    }, [inventory, setGold]);
    ...
    const bulkSellInventorySupports = useCallback((supportTypeFilter = null) => {
        ...
        setSupportInventory(prev => prev.filter(s => !sellIds.has(s.id)));
        setGold(g => g + refund);
        setSelectedSupportInventory([]);
    }, [supportInventory, setGold]);
