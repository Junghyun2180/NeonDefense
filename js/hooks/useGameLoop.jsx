...
const useGameLoop = (config) => {
    ...
    const {
        ...
        stage,
        wave,
        sector = 1, // 합의 10: 캠페인 sector (HP 스케일 곱연산)
        gold,
        lives,
    ...
    useEffect(() => {
        ...
        spawnIntervalRef.current = setInterval(() => {
            ...
            const newEnemy = EnemySystem.create(stage, wave, localSpawnedCount, totalEnemies, selectedPath.tiles, selectedPath.id, modeAbility, sector);
            if (newEnemy) {
                // 순환 경로인 경우 표시
                if (isLooping) {
