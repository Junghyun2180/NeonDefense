...
| 08 | [회의록: 사우론 타워 합의안](./08-meeting-saurom-tower.md) | 합의 / 롤아웃 | 디자이너(분리)·밸런서(부분 도입) 충돌 → **유한 캠페인 (3 floors × 3 stages × 10 waves = 90 waves) + 무한 등반은 별도 모드** 합의. (※ 합의 10 에서 정정 — 1 run = 30W) |
| 09 | [모바일 친화 일시정지/저장 시스템](./09-mobile-pause-save.md) | 모바일 UX / 인프라 | Pause(즉시) + 웨이브 단위 자동저장 + 백그라운드 자동 pause 3종 정책. `useGameLoop` paused 분기 + `save-system v2` + `visibilitychange` 훅. MVP/V2/V3 분리. |
| 10 | [회의록: 사우론 타워 정정 합의안](./10-meeting-saurom-tower-corrected.md) | 합의 / 롤아웃 (정정) | 사용자 정정 반영 — **1 run = 30W (15~20분)**, **Floor 는 런 외부 progression**. 메타 신규 4종 (`floorBonusGold`/`damageVsHighFloor`/`crystalMultiplier`/`baseDamage maxLv 50`) + Pause MVP 동시 도입 필수. 합의 08 폐기. |
...
| 04 | [Floor 메타 + 영구 스탯 곡선 검증](./balance-04-floor-meta-curve.md) | 정정 합의(10) 전제. `META_UPGRADES` 8종이 floor 시스템 미전제 — `baseDamage +30%` 가 Floor 5(×1.75) 못 따라잡음. **부분 도입 + 메타 4종 신규** 권고 (`floorBonusGold`/`damageVsHighFloor`/`crystalMultiplier`/`baseDamage maxLv 50`). |
